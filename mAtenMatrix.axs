MODULE_NAME='mAtenMatrix' (DEV vdvControl, DEV ipDevice)
...
DEFINE_FUNCTION fnSendFromQueue(){
	...
	ELSE IF(myMatrix.CONN_STATE == CONN_STATE_CONNECTED && FIND_STRING(myMatrix.Tx,"$0D,$0A",1)){
		...
		fnDebug(DEBUG_STD,'->aten',_ToSend);
...
DEFINE_EVENT DATA_EVENT[vdvControl]{
	ONLINE:{
		SEND_STRING DATA.DEVICE,"'PROPERTY-META,MAKE,aten'"
	...
	COMMAND:{
		SWITCH(fnStripCharsRight(REMOVE_STRING(DATA.TEXT,'-',1),1)){
			...
			CASE 'MATRIX':{
				STACK_VAR CHAR MTX_I[2]
				STACK_VAR CHAR MTX_O[2]
				MTX_I = fnStripCharsRight(REMOVE_STRING(DATA.TEXT,'*',1),1)
				MTX_O = DATA.TEXT
				fnAddToQueue("'sw i0',MTX_I,' o0',MTX_O")
				}
			}
		}
	}
